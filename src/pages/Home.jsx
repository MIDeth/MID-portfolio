export default function Home() {
  return (
    <>
      <div className="w-[35rem] font-main ml-20 max-md:ml-32 mb-12">
        <div className="mt-2 mb-7">
          <h1 className="text-6xl font-vogue">MID</h1>
        </div>
        <div>
          I’m a frontend, web3 developer, optimist, and community builder. I
          currently work in a team that has experience at PEPE, WallSTMemes, and
          DORKL.
        </div>
        <div className="flex items-center pl-6 my-12">
          <div className="w-[5rem]">
            <img src="logo/logo.svg" className="" />
          </div>
          <div className="flex flex-col ml-10">
            <a
              href="https://twitter.com/MIDeth_"
              target="_blank"
              rel="noreferrer"
            >
              <div className="flex items-center">
                <div className="w-[1.4rem] my-1 align-middle inline-block">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 512 512"
                    fill="#eeeded"
                  >
                    <path d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z" />
                  </svg>
                </div>
                <div className="inline-block ml-2 mt-[.2rem] align-middle">
                  Twitter
                </div>
              </div>
            </a>
            <a
              href="https://github.com/MIDeth"
              target="_blank"
              rel="noreferrer"
            >
              <div className="flex">
                <div className="w-[1.4rem] my-1">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 496 512"
                    fill="#eeeded"
                  >
                    <path d="M165.9 397.4c0 2-2.3 3.6-5.2 3.6-3.3.3-5.6-1.3-5.6-3.6 0-2 2.3-3.6 5.2-3.6 3-.3 5.6 1.3 5.6 3.6zm-31.1-4.5c-.7 2 1.3 4.3 4.3 4.9 2.6 1 5.6 0 6.2-2s-1.3-4.3-4.3-5.2c-2.6-.7-5.5.3-6.2 2.3zm44.2-1.7c-2.9.7-4.9 2.6-4.6 4.9.3 2 2.9 3.3 5.9 2.6 2.9-.7 4.9-2.6 4.6-4.6-.3-1.9-3-3.2-5.9-2.9zM244.8 8C106.1 8 0 113.3 0 252c0 110.9 69.8 205.8 169.5 239.2 12.8 2.3 17.3-5.6 17.3-12.1 0-6.2-.3-40.4-.3-61.4 0 0-70 15-84.7-29.8 0 0-11.4-29.1-27.8-36.6 0 0-22.9-15.7 1.6-15.4 0 0 24.9 2 38.6 25.8 21.9 38.6 58.6 27.5 72.9 20.9 2.3-16 8.8-27.1 16-33.7-55.9-6.2-112.3-14.3-112.3-110.5 0-27.5 7.6-41.3 23.6-58.9-2.6-6.5-11.1-33.3 2.6-67.9 20.9-6.5 69 27 69 27 20-5.6 41.5-8.5 62.8-8.5s42.8 2.9 62.8 8.5c0 0 48.1-33.6 69-27 13.7 34.7 5.2 61.4 2.6 67.9 16 17.7 25.8 31.5 25.8 58.9 0 96.5-58.9 104.2-114.8 110.5 9.2 7.9 17 22.9 17 46.4 0 33.7-.3 75.4-.3 83.6 0 6.5 4.6 14.4 17.3 12.1C428.2 457.8 496 362.9 496 252 496 113.3 383.5 8 244.8 8zM97.2 352.9c-1.3 1-1 3.3.7 5.2 1.6 1.6 3.9 2.3 5.2 1 1.3-1 1-3.3-.7-5.2-1.6-1.6-3.9-2.3-5.2-1zm-10.8-8.1c-.7 1.3.3 2.9 2.3 3.9 1.6 1 3.6.7 4.3-.7.7-1.3-.3-2.9-2.3-3.9-2-.6-3.6-.3-4.3.7zm32.4 35.6c-1.6 1.3-1 4.3 1.3 6.2 2.3 2.3 5.2 2.6 6.5 1 1.3-1.3.7-4.3-1.3-6.2-2.2-2.3-5.2-2.6-6.5-1zm-11.4-14.7c-1.6 1-1.6 3.6 0 5.9 1.6 2.3 4.3 3.3 5.6 2.3 1.6-1.3 1.6-3.9 0-6.2-1.4-2.3-4-3.3-5.6-2z" />
                  </svg>
                </div>
                <div className="inline-block ml-2 mt-[.28rem] align-middle">
                  GitHub
                </div>
              </div>
            </a>
            <a href="https://t.me/MID_whale" target="_blank" rel="noreferrer">
              <div className="flex ">
                <div className="w-[1.4rem] my-1">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 48 48"
                    fill="#eeeded"
                  >
                    <path d="M5.83,23.616c12.568-5.529,28.832-12.27,31.077-13.203c5.889-2.442,7.696-1.974,6.795,3.434 c-0.647,3.887-2.514,16.756-4.002,24.766c-0.883,4.75-2.864,5.313-5.979,3.258c-1.498-0.989-9.059-5.989-10.7-7.163 c-1.498-1.07-3.564-2.357-0.973-4.892c0.922-0.903,6.966-6.674,11.675-11.166c0.617-0.59-0.158-1.559-0.87-1.086 c-6.347,4.209-15.147,10.051-16.267,10.812c-1.692,1.149-3.317,1.676-6.234,0.838c-2.204-0.633-4.357-1.388-5.195-1.676 C1.93,26.43,2.696,24.995,5.83,23.616z"></path>
                  </svg>
                </div>
                <div className="inline-block ml-2 mt-[.28rem] align-middle">
                  Telegram
                </div>
              </div>
            </a>
          </div>
        </div>
        <div>
          I’m a frontend and web3 developer and help the crypto project owner
          team in dev and marketing staff. I’m passionate about frontend
          development and have experience in crypto projects on the dev side
          writing smart contracts and building DAPPs, and website for it. I’m an
          advisor and investor in early-stage startups
        </div>
        <div className="flex mt-10">
          <div className="flex mx-3">
            <div className="w-[1rem] mr-2 mt-[.10rem]">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 448 512"
                fill="#eeeded"
              >
                <path d="M384 32c35.3 0 64 28.7 64 64V416c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V96C0 60.7 28.7 32 64 32H384zM160 144c-13.3 0-24 10.7-24 24s10.7 24 24 24h94.1L119 327c-9.4 9.4-9.4 24.6 0 33.9s24.6 9.4 33.9 0l135-135V328c0 13.3 10.7 24 24 24s24-10.7 24-24V168c0-13.3-10.7-24-24-24H160z" />
              </svg>
            </div>
            <p className="inline-block mb-1 align-middle">
              <a href="mailto:">contact me</a>
            </p>
          </div>
          <div className="flex mx-3">
            <div className="w-[1rem] mr-2 mt-[.10rem]">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 448 512"
                fill="#eeeded"
              >
                <path d="M384 32c35.3 0 64 28.7 64 64V416c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V96C0 60.7 28.7 32 64 32H384zM160 144c-13.3 0-24 10.7-24 24s10.7 24 24 24h94.1L119 327c-9.4 9.4-9.4 24.6 0 33.9s24.6 9.4 33.9 0l135-135V328c0 13.3 10.7 24 24 24s24-10.7 24-24V168c0-13.3-10.7-24-24-24H160z" />
              </svg>
            </div>
            <a
              href="https://github.com/MIDeth/MID-portfolio"
              target="_blank"
              rel="noreferrer"
            >
              <p>source-code</p>
            </a>
          </div>
        </div>
      </div>
    </>
  );
}
